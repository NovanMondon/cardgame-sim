...
export const simulate = (
  ...
  type ResultRecord = { winner: 0 | 1, reason: string }
  ...
  for (const key of Object.keys(gameResultCount)) {
    const result: ResultRecord = JSON.parse(key);
  ...
  output(`
    ...
    Time: Total ${(endTime - startTime)} ms
      ${(endTime - startTime) / count * 1000000} ns/game
